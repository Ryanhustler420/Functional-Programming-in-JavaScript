...
repeat(exclaim(scream('I love JavaScript'))); // I LOVE JAVASCRIPT! I LOVE JAVASCRIPT!
...
// When you wanna execute sequence of functions in order than use this function

const compose = (...fns) => {
  console.log(fns); // [repeat, exclaim, scream]
  return x => {
    console.log(x); // i love JavaScript
    return fns.reduceRight((_x, fn) => {
      console.log(_x);
      console.log(fn);
      return fn(_x);
    }, x);
...
const enhanceWithPipe = pipe(
  scream,
  exclaim,
  repeat
);

enhanceWithPipe('I love JavaScript'); /*? */
